...
bool vk_init(VkContext* vkcontext, void* window) 
{
    ...
    //Choose GPU
    {
        ...
        for(uint32_t i = 0; i < gpuCount; i++)
        {
            ...
            {
                if(queueProps[j].queueFlags & VK_QUEUE_GRAPHICS_BIT)//finding a queue thats capable of doing graphics
                {                                                  //also checking if gpu can present images to our surface
    ...
    // Logical Device
    {
        ...
        VK_CHECK(vkCreateDevice(vkcontext->gpu, &deviceInfo, 0, &vkcontext->device)); //creates logical device
    }

    //swapchain kinda manages the images between frames, requiring a lot of synchronization
    {
